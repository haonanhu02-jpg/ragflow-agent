...
baseline_project_root: "D:/download/myself"
current_project_root: "D:/download/ragflow-agent"
last_revalidated_at: "2026-07-30"
---
...
- **[历史事实]** Phase 00 出口时 Phase 01 为“待确认/未执行”，Phase 02 至 Phase 10 为“预规划草案/未执行”；当前增量状态见第 6 节。
...
| `docs/phases/phase-01-project-skeleton.md` | Phase 01 详细计划 | Phase 00 快照时待确认/未执行；当前状态见第 6 节 |
...
| 能力层 | 当前事实 | 进入实现所需前置 |
|---|---|---|
| 工程骨架 | 不存在 | Phase 00 完成、O-001 解决、Phase 01 计划确认；三项现均满足，下一执行任务为 P01-T02 |
...
- **[事实]** 当前项目根目录为 `D:/download/ragflow-agent`，已初始化为 Git 仓库。
- **[事实]** 默认分支为 `main`，远程 `origin` 实际配置为 `https://github.com/haonanhu02-jpg/ragflow-agent.git`；P01-T01 开始前 `HEAD` 与 `origin/main` 均为 `5c015405e4c25346999cbb21736c61a87d5f8cbe`。
- **[事实]** Phase 00 基线已推送；P01-T01 的未提交改动只涉及 Markdown 规划与执行记录。
- **[决策]** ADR-016 已冻结项目名、包名、服务名、GitHub Actions 和 `mypy`。
- **[历史事实]** P01-T01 完成时，P01-T02 至 P01-T10 尚未执行。
- **[历史事实]** P01-T01 完成时，`src/`、`tests/`、`migrations/`、`pyproject.toml`、`uv.lock`、`.github/` 和部署文件仍不存在；业务实现进度为 0。

## 6. P01-T02 增量快照

- **[历史事实]** P01-T02 完成时，项目已有 `.gitignore`、`pyproject.toml`、`uv.lock`、`src/ragflow_agent/__init__.py` 和 `tests/unit/test_package.py`。
- **[历史事实]** P01-T02 完成时，项目 `.venv` 使用 Python 3.13.5，依赖由 `uv` 安装和锁定；`.venv`、bytecode 与质量工具缓存均被 Git 忽略。
- **[历史事实]** P01-T02 完成时已配置 pytest、ruff 和 strict mypy；同步、导入、测试、lint、类型、锁文件和包兼容性验证均通过。
- **[历史事实]** P01-T02 完成时尚无 FastAPI 路由或服务、Ingestion Worker、配置 Schema、数据库迁移、Agent 或 RAG 业务实现。
- **[历史事实]** P01-T02 完成时，P01-T01、P01-T02 已完成，P01-T03 至 P01-T10 尚未执行。

## 7. Phase 01 出口状态

- **[事实]** P01-T01 至 P01-T10 已全部完成并通过阶段验收。
- **[事实]** 当前已有可安装包、配置/观测/基础设施边界、Alembic 空迁移、FastAPI/Worker 空壳、Docker 开发拓扑和 GitHub Actions 门禁。
- **[事实]** 当前仍无 Agent、知识库、真实 ingestion、Parser、索引、检索或 RAG 业务能力。
