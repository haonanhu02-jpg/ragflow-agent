...
- 审计执行时的目标项目目录：`D:/download/myself`；当前 Git 项目根目录已迁移为 `D:/download/ragflow-agent`。
...
| 项目 | 影响 | 处理 |
|---|---|---|
| O-001 项目正式名称和 Python 包名 | 当时阻止 Phase 00 出口和 Phase 01 准入 | 当时继续使用旧包路径占位且不得创建包；该历史门禁后来由 ADR-016 解决 |
