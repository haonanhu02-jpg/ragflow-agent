...
- 当前状态必须按已验证的阶段事实记录：Phase 00 至 Phase 06 已完成；Phase 07 及以后能力仍只能按实际前置实现标注。
...
| CAP-18 | 候选清理与去重 | 是 | `Dealer._prune_deleted_chunks`；检索合并和 Chunk ID 去重 | 部分 | 否 | 自研 CandidateCleaner；后端先排除禁用/删除，应用层按 ID/阈值/每文档限额清理 | 参考重写 | Phase 06 | 状态、重复、阈值、每文档限额和最终截断测试 | 已完成 Phase 06 清理；旧版本最终可见性和物理删除行为仍由 Phase 07 生命周期保障 |
...
| CAP-21 | 引用与来源定位 | 是 | `generator.py::kb_prompt/citation_prompt`；`Dealer.fetch_chunk_vectors/insert_citations`；`dialog_service.py::repair_bad_citation_formats` | 部分：Prompt | 否 | 沿用自研 Citation，统一在线检索只输出同授权/版本来源 | 参考重写 | Phase 04 基础；Phase 05 bbox；Phase 06 完整 | 引用存在性、页码/bbox/quote/版本/权限和回答公开字段 | 已完成在线主链路 Citation/source/trace_id；删除后可见性和准确率大数据集分别留 Phase 07/10 |
