...
本标准适用于源码、测试、数据库迁移、配置、Prompt、评测数据、部署文件和文档。当前项目已完成 Phase 01 至 Phase 10 的规划内任务；后续生产接入、运营验证和维护仍受本文件约束。
...
1. 本地、Fake、Stub、协议、隔离基础设施、真实 Provider 和真实生产验证必须分别报告。
2. 短时测试不能证明月度 SLO；合成数据不能证明真实业务质量；内容寻址恢复测试不能冒充生产备份恢复。
3. 本地或自有云发布报告对质量、安全和隔离恢复门禁保持 fail closed；模型密钥由用户运行时提供，企业 IdP/业务数据、长期 SLO、Docker Scout、ARM64、UI 和项目顶层 LICENSE 不作为当前代码完成门禁。
4. Linux/amd64、linux/arm64、TLS、观测后端和外部依赖只有实际执行成功后才能标记已验证。
5. UI/管理控制台为 Deferred，不能以 API 占位或文档描述成已实现。
6. 项目有意不设置顶层 `LICENSE`；不得删除依赖、数据集、模型、外部资源或 vendored 第三方内容依法必须保留的许可证与 NOTICE。
